...
require_csrf();
// 确保 cp_comments 表有 reply 和 replied_at 字段
try { db()->exec("ALTER TABLE cp_comments ADD COLUMN reply TEXT DEFAULT NULL"); } catch (Exception $e) {}
try { db()->exec("ALTER TABLE cp_comments ADD COLUMN replied_at DATETIME DEFAULT NULL"); } catch (Exception $e) {}
...
.user-table th{padding:10px 8px;text-align:left;font-weight:700;color:var(--tl);font-size:.78em;white-space:nowrap;border-bottom:2px solid rgba(0,0,0,.04)}
.user-table td{padding:10px 8px;border-bottom:1px solid rgba(0,0,0,.04);vertical-align:middle;white-space:nowrap}
.user-table tbody tr:hover{background:rgba(212,120,110,.03)}
...
.modal-box{background:#fff;border-radius:20px;box-shadow:0 8px 40px rgba(0,0,0,.15);width:480px;max-width:92vw;max-height:90vh;overflow-y:auto;padding:24px}
...
.user-table tbody tr{display:block;margin-bottom:10px;background:#fff;border-radius:10px;box-shadow:0 1px 6px rgba(0,0,0,.04);padding:10px 12px}
...
.user-table tbody tr:hover{background:#fff}
