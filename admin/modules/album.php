...
?>
<?php if ($tab === 'album'): ?>
<div class="card"><div class="card-title"><?php echo m_ico_badge('upload'); ?>上传照片</div>
<form method="post" enctype="multipart/form-data"><?php echo csrf_field(); ?><input type="hidden" name="act" value="save_photo">
<div class="fg"><label><span class="lbl-ico"><?php echo m_ico("tag", 15); ?></span> 描述</label><input type="text" name="title" class="neo" placeholder="例如：第一次约会"></div>
<div class="fg"><label><span class="lbl-ico"><?php echo m_ico("camera", 15); ?></span> 照片</label><input type="file" name="photo[]" accept="image/*" multiple required></div>
<div class="btn-group"><button type="submit" class="btn primary"><span class="lbl-ico"><?php echo m_ico("camera", 15); ?></span> 上传</button></div></form></div>
<div class="card"><div class="card-title"><?php echo m_ico_badge('album'); ?>相册 (<?php echo count($photos);?>张)</div>
