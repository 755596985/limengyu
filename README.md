...
> 💡 推荐路径：使用本步的 **install.php 安装向导**。向导会自动导入 `schema.sql` 建表、创建管理员并写入数据库配置，因此第 1~2 步（手动导库/手填配置）可以跳过；若你已经手动初始化过，向导也能安全识别并仅补齐管理员与站点配置（schema 全部幂等）。

1. 将全部源码上传到网站根目录（确保 `data/`、`uploads/` 目录可写）
2. 浏览器访问 `https://你的域名/install.php`
3. 按向导依次填写：**数据库连接信息**（自动导入 schema.sql 建表）、**管理员账号密码**、**你们的名字**、**纪念日**、**网站标题**；若目录可写，向导会自动把连接信息写入 `include/config.db.php`
4. 安装完成后 **立即删除 `install.php`**，然后访问后台 `/admin/index.php` 开始装扮你们的小窝

管理员账号为安装向导中设置的账号；若跳过向导手动初始化，则首次访问后台会自动创建 `admin / admin123`（**请登录后立即修改**）
