...
function renderPostCard($po, $CM, $n1, $n2, $a1, $a2, $me) {
    ...
    if ($isUserPost) {
        $pu = user_by_id($po['user_id']);
        $pav = $pu ? ($pu['avatar'] ?? '') : '';
        $pem = '👤';
        $pname = '<a href="user.php?id='.htmlspecialchars($po['user_id']).'" style="color:inherit;text-decoration:none;">'.htmlspecialchars($po['user_nick'] ?? '用户').'</a>';
        $pcolor = '#888';
    } else {
